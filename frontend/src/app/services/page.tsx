...
const ServiceMap = dynamic(() => import('@/components/ServiceMap'), {
  ssr: false,
  loading: () => <div className="h-[400px] w-full bg-gray-100 rounded-3xl animate-pulse" />
});
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center p-8 max-w-md">
          ...
          <button
            ...
            className="px-6 py-2 bg-yellow-500 text-white font-bold rounded-xl hover:bg-yellow-600 transition-colors"
          >
            Réessayer
          </button>
        </div>
        ...
        <div className="mb-10 space-y-6">
          ...
          <div className="relative max-w-md">
            ...
            <input
              ...
              className="w-full pl-12 pr-4 py-3 bg-white border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500 transition-all shadow-sm"
            />
          </div>
        </div>
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all duration-200 border ${selectedCategory === cat
                ? "bg-yellow-500 text-white border-yellow-500 shadow-lg shadow-yellow-500/25 transform scale-105"
    ...
    <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-custom-sm hover:shadow-custom-lg transition-all duration-300 group relative overflow-hidden">
      ...
      <div className="flex items-start gap-4 mb-6">
        <div className="w-14 h-14 bg-gray-50 rounded-2xl flex items-center justify-center text-3xl shadow-sm group-hover:scale-110 transition-transform duration-300">
          <IconComponent className="w-8 h-8 text-gray-700 group-hover:text-yellow-500 transition-colors" />
        </div>
        <div className="pt-1">
          <h3 className="font-bold text-lg text-gray-900 leading-tight mb-1 group-hover:text-yellow-500 transition-colors">
            {service.name}
          </h3>
      ...
      {/* AI Insight (New) */}
      <div className="mb-6 bg-yellow-50 rounded-xl p-3 border border-yellow-100 flex items-start gap-2">
        <Sparkles className="w-4 h-4 text-yellow-600 mt-0.5 flex-shrink-0" />
        <div>
          <p className="text-xs font-bold text-yellow-900">IA Insight</p>
      ...
      <button
        ...
        className={`w-full py-3.5 rounded-xl font-bold flex items-center justify-center gap-2 transition-all duration-300 ${isOpen
          ? "bg-gray-900 text-white hover:bg-yellow-500 hover:text-white hover:shadow-lg hover:-translate-y-0.5"
        ...
        <div className="p-8 space-y-8 overflow-y-auto max-h-[80vh]">
            ...
            <div className="h-40 w-full bg-white rounded-xl border border-gray-100 p-2 shadow-sm">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={[
                  ...
                  />
                  <Area type="monotone" dataKey="wait" stroke="#FDB913" strokeWidth={3} fillOpacity={1} fill="url(#colorWait)" />
                </AreaChart>
              </ResponsiveContainer>
            </div>
              ...
              <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-2">
                {selectedService.required_documents.map((doc, idx) => (
                  <div key={idx} className="flex items-start gap-3">
                    <div className={`mt-0.5 w-4 h-4 rounded-full border flex items-center justify-center flex-shrink-0 ${doc.required ? 'border-yellow-500 bg-yellow-500 text-white' : 'border-gray-400'}`}>
          ...
          <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-gray-900 text-sm">Estimation IA</span>
              </div>
              {loadingPrediction && (
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 border-2 border-yellow-600 border-t-transparent rounded-full animate-spin"></div>
                  <span className="text-xs text-yellow-600 font-medium">Analyse...</span>
                </div>
              )}
            </div>
            ...
            {!loadingPrediction && !prediction && (
              <p className="text-sm text-gray-400 italic">Aucune prédiction disponible pour le moment.</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <button
              ...
              className="flex-[2] py-4 px-6 bg-yellow-500 text-white rounded-xl font-bold shadow-lg shadow-yellow-500/25 hover:bg-yellow-600 hover:shadow-xl hover:-translate-y-0.5 transition-all"
            >
              Confirmer le ticket
            </button>
          </div>
